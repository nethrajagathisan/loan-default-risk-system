...
class LoanExplainer:
    def __init__(self,
        ...
        try:
            self.feature_names = joblib.load(base_dir / "models/feature_names.joblib")
        except Exception:
...
def load_test_data(test_path: str = "data/processed/model_df.parquet") -> Tuple[pd.DataFrame]:
    """Load the processed dataset produced by src/data_processing.py"""

    if not os.path.exists(test_path):
        raise FileNotFoundError(
            f"{test_path} not found. Place the raw Kaggle CSV at data/raw/train.csv "
            "and run: python src/data_processing.py"
        )
